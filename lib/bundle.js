	...
	var Grid = function () {
	  function Grid(sequencer) {
	    ...
	    if ($(window).width() >= 1075) {
	      this.config.canvasWidth = Math.ceil(($('.game-container').width() - 10) / cellWidth) * cellWidth;
	    } else if ($(window).width() < 461) {
	      this.config.cellWidth = 25;
	      // have to divide and multiply to make sure borders are even
	      this.config.canvasWidth = Math.ceil(($('.game-container').width() - 10) / 25) * 25;
	      this.config.canvasHeight = Math.ceil(($('.game-container').height() - 110) / 25) * 25;
	    } else {
	      this.config.canvasWidth = Math.ceil(($('.game-container').width() - 10) / cellWidth) * cellWidth;
	    }
	  ...
	  _createClass(Grid, [{
	    ...
	    value: function initialize() {
	      $("#background, #easel").attr({
	        "height": '' + this.config.canvasHeight,
	        "width": '' + this.config.canvasWidth
	      });
	
	      $('.canvas-container').css("width", '' + this.config.canvasWidth);
